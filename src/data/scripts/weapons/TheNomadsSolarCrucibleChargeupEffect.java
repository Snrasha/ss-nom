...
public class TheNomadsSolarCrucibleChargeupEffect implements EveryFrameWeaponEffectPlugin
{
    ...
    public void advance( float amount, CombatEngineAPI engine, WeaponAPI weapon )
    {
        ...
        if( weapon.isFiring() )
        {
			...
			if( progress == 0.0f )
			{
				Vector2f explosion_offset = trylobot.translate_polar( weapon_location, OFFSET + ((0.25f * 150f) - 2f), ship.getFacing() );
				engine.spawnExplosion( explosion_offset, ship.getVelocity(), MUZZLE_FLASH_COLOR, 150f, 0.3f );
			}
            // particles
            Vector2f particle_offset = trylobot.translate_polar( weapon_location, OFFSET, ship.getFacing() );
            ...
            {
				size = trylobot.get_random( 1f, 10f );
                speed = trylobot.get_random( 200f, 800f );
                angle = ship.getFacing() + trylobot.get_random( -20f, 20f );
                velocity = trylobot.translate_polar( ship.getVelocity(), speed, angle );
